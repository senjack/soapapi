...
class CohortSerializer (serializers.HyperlinkedModelSerializer) :
    class Meta:
        model = Cohort
        fields = ('cohort_id', 'name_name', 'registration_date', 'start_date', 'end_date', 'cohort_number', 'description', 'running')

class CatalystSerializer (serializers.HyperlinkedModelSerializer) :
    class Meta:
        model = Catalyst
        fields = ('catalyst_id', 'name_name', 'registration_date', 'start_date', 'end_date', 'catalyst_number', 'description', 'running')

class BootcampSerializer (serializers.HyperlinkedModelSerializer) :
    class Meta:
        model = Bootcamp
        fields = ('bootcamp_id', 'name_name', 'registration_date', 'start_date', 'end_date', 'bootcamp_number', 'description', 'running')

class ContactSerializer (serializers.HyperlinkedModelSerializer) :
    class Meta:
        model = Contact
        fields = ('applicant_id', 'linkedin_link', 'twitter_link', 'facebook_link')

class EducationBackgroundSerializer (serializers.HyperlinkedModelSerializer) :
    class Meta:
        model = EducationBackground
        fields =('applicant_id', 'school', 'degree', 'Field_of_study', 'grade', 'From', 'to', 'Extra_activity')
...
class HonorSerializer (serializers.HyperlinkedModelSerializer):
    class Meta: Honor
    fields = ('applicant_id', 'honor_title', 'association', 'Issuer', 'date_of_honor', 'Description')

class ApplicationVideoSerializer (serializers.HyperlinkedModelSerializer):
    class Meta: ApplicationVideo
